...
public class BitwiseOperator {
	...
	public static void main(String[] args) throws IOException {
		 ...
		 * 
		 * String[] arr = { "vikram", "vnjdadn", "vmhdsu", "v122" }; String word =
		 * findLongestprefix(arr); String
		 * output=word.trim().length()>0?"this array  has a common prefix =>"
		 * +word:"this array has no common prefix=> -1"; System.out.println(output);
		 */
...
//	quest();
	...
	/*
	 * public static void quest() { String s =
	 * "@#$%^^&*ab@#&*#%@!cd255kik5*&lk()989"; StringBuffer newStr=new
	 * StringBuffer(s).reverse(); System.out.println(newStr); String firstChar="";
	 * String secondChar=""; int firstVal=0; int secondVal=0; boolean flag = false;
	 * for (int i = firstVal; i < s.length(); i++) { char c = s.charAt(i); if ((c >=
	 * 65 && c <= 90) || (c >= 97 && c <= 122)) { firstChar = String.valueOf(c);
	 * firstVal = i; for (int j = secondVal; j < newStr.length(); j++) { char d =
	 * newStr.charAt(j); if ((d >= 65 && d <= 90) || (d >= 97 && d <= 122)) {
	 * secondChar = String.valueOf(d); secondVal = j+1;
	 * s=s.replace(firstChar,secondChar); break; } } }
	 * 
	 * 
	...
	public static void set() {
		...
		sb.setCharAt(0, 'm');
		System.out.println("this is setChar at=>" + sb);
	}
